<?php
/**
 * Administrative Console for Corviprotect
 * This seems to be the old version of cpnc, but I still use it in unit tests
 * because of handleCommand, which works directly on the database. Most likely
 * from the first concept when Corviprotect was entirely local for getting first
 * concepts done.
 * I should move this one to tests.
 *
...
class CPAdm {
	...
	function getCommand(string $input = ""): CommandParser {
		while($input=="") {
			#echo "cpmad> ";
	...
	function handleCommand(CommandParser $command): void {
